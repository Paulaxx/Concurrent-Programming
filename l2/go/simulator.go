...
func vertex_routine(v *vertex, chans [n]chan *packagee, printer chan<- string, pickup chan<- *packagee, traps [n]chan string) {
...
func poacher(traps [n]chan string) {

	for {
		delayy := rand.Intn(2) + 1
		time.Sleep(time.Duration(delayy) * time.Second)

		rand := rand.Intn(n - 1)
		traps[rand] <- "It's a trap"
	}
...
func main() {
	...
	var vis []int
	for i := 0; i < k; i++ {
		pack := packagee{id: i, step: 0, dead: false, visited_vertices: vis, trap: false}
		p[i] = &pack
	}
	...
	var traps [n]chan string
	for i := range traps {
		traps[i] = make(chan string, k)
	}
	...
	for i := 0; i < n-1; i++ {
		...
		v[i] = &x
		go vertex_routine(&x, chans, printer, pickup, traps)
	}
